...
export default class Register extends Component {
  ...
  createUserAccount = () => {
    ...
    this.setState({
      ...
      if(name !== '' && email !== '' && password !== '' && (repeat !== '' && repeat === password)){
		  if(name === null || name.split(" ") === null || name.split(" ").length !== 2 || name.split(" ")[0].length < 1 || name.split(" ")[1].length < 1) {
				message = "Please use your full name";
				this.setState({...this.state, showError: true, errorMessage: message});
		  } else {
				this.createFireBaseAccount(name, email, password, this.state.faithIndex);
		  }
      } else {
  ...
  createFireBaseAccount = (name, email, password, faith) => {
    ...
    Firebase.createFirebaseAccount(name, email, password, faith, this.registerError.bind(this), this.props.onRegister)
      .then(result => {
  ...
  render() {
	var catholic = () => <Text>Catholic</Text>
	var christian = () => <Text>Christian</Text>
	var nonreligious = () => <Text>Nonreligious</Text>
	var other = () => <Text>Other</Text>
	var buttons = [{element: catholic}, {element: christian}, {element: nonreligious}, {element: other}];
    return(
